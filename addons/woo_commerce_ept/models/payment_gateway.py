...
class WooPaymentGateway(models.Model):
    ...
    woo_instance_id = fields.Many2one("woo.instance.ept", string="Instance", required=False)
    account_paypal_payment = fields.Many2one('asiup.paypal.integration',string='Paypal Account')
    ...
    def create(self, vals_list):
        res = super(WooPaymentGateway, self).create(vals_list)
        if res.woo_instance_id:
            res.woo_instance_id.prepare_payment_methods()
        return res
