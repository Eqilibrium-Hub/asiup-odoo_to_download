...
class WooInstanceEpt(models.Model):
    ...
    name = fields.Char(compute='compute_name_instance')
    ...
    def name_get(self):
        return [(rec.id, rec.woo_host.split('//')[-1]) for rec in self]

    def compute_name_instance(self):
        for rec in self:
            rec.name = rec.display_name
            if not rec.primary_domain:
    ...
    def get_refund(self):
        ...
        """
        refund_query = """select id from refund_request where woo_instance_id=%s""" % self.id

        def refund_of_current_week(refund_query):
            qry = refund_query + " and date(refund_woo_at) >= (select date_trunc('week', date(refund_woo_at)))"
            self._cr.execute(qry)
            return self._cr.dictfetchall()

        def refund_of_current_month(refund_query):
            qry = refund_query + " and date(refund_woo_at) >= (select date_trunc('month', date(refund_woo_at)))"
            self._cr.execute(qry)
            return self._cr.dictfetchall()

        def refund_of_current_year(refund_query):
            qry = refund_query + " and date(refund_woo_at) >= (select date_trunc('year', date(refund_woo_at)))"
            self._cr.execute(qry)
            return self._cr.dictfetchall()
        ...
        view = self.env.ref('asiup_shopbase_integration.refund_request_action').read()[0]
    ...
    def woo_connect(self):
        ...
        woo_version = self._context.get("woo_version") or self.woo_version
        wc_api = woocommerce.api.API(url=host, consumer_key=consumer_key, consumer_secret=consumer_secret,
                                     verify_ssl=self.woo_verify_ssl, version=woo_version, query_string_auth=True)
        return wc_api
    ...
    def configure_woo_order_webhook(self):
        ...
        @author: Maulik Barad on Date 06-Jan-2019.
        """
        topic_list = ["order.updated", "order.deleted", "order.created"]
        self.configure_webhooks(topic_list)
        return True
