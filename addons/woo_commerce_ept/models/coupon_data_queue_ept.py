...
class WooCouponDataQueueEpt(models.Model):
    ...
    def import_coupon_queue(self):
        self.env['woo.manual.queue.process.ept'].with_context(queue_type='coupon', active_ids=[self.id]).process_queue_manually()
    ...
    def create(self, vals):
        ...
        res = super(WooCouponDataQueueEpt, self).create(vals)
        res.with_delay(description='Sync coupon from Woo').import_coupon_queue()
        return res
