...
class SaleOrder(models.Model):
    ...
    updated_in_woo = fields.Boolean("Updated In woo", compute="_get_woo_order_status", search="_search_woo_order_ids",
                                    copy=False)
    cancelled_in_woo = fields.Boolean("Cancelled In WooCommerce", default=False, copy=False)
    woo_status = fields.Selection([("pending", "Pending"), ("processing", "Processing"),
                                   ("on-hold", "On hold"), ("completed", "Completed"),
                                   ("cancelled", "Cancelled"), ("refunded", "Refunded"),
                                   ("failed", "Failed"), ("trash", "Trash")], copy=False, tracking=7,
                                  string='Woo Status')
    ...
    date_created_gmt = fields.Datetime(string="Date GMT")
    primary_domain = fields.Char(string='Primary Domain', compute='_compute_primary_domain', store=True)
    ...
    @api.depends('shopbase_store_id', 'woo_instance_id')
    def _compute_primary_domain(self):
        for rec in self:
            if rec.woo_instance_id:
                rec.primary_domain = rec.woo_instance_id.display_name
            else:
                rec.primary_domain = rec.shopbase_store_id.display_name

    @api.model
    def create(self, vals_list):
        res = super(SaleOrder, self).create(vals_list)
        res.update_sale_order_status()
        if res.woo_order_id:
            order_checkout_url = res.woo_instance_id.woo_host + '/wp-admin/post.php?post=' + str(res.woo_order_id) + '&action=edit'
            res.order_status_url = order_checkout_url

        return res

    def sale_order_processing(self):
        self.order_status = 'processing'
        self.fulfillment_status = 'unexported'
        self.payment_status = 'paid'
        for line in self.order_line:
            line.sol_status = 'processing'
            line.sol_payment_status = 'paid'
            line.sol_fulfillment_status = 'unexported'

    def sale_order_on_hold(self):
        self.order_status = 'on_hold'
        self.fulfillment_status = 'pending_export'
        self.payment_status = False
        for line in self.order_line:
            line.sol_status = 'on_hold'
            line.sol_payment_status = False
            line.sol_fulfillment_status = 'pending_export'

    def sale_order_on_pending(self):
        self.order_status = 'pending_payment'
        self.fulfillment_status = 'pending_export'
        self.payment_status = 'pending'
        for line in self.order_line:
            line.sol_status = 'pending_payment'
            line.sol_payment_status = 'pending'
            line.sol_fulfillment_status = 'pending_export'

    def sale_order_on_other(self):
        self.order_status = 'failed'
        self.fulfillment_status = 'pending_export'
        self.payment_status = False
        for line in self.order_line:
            line.sol_status = 'failed'
            line.sol_payment_status = False
            line.sol_fulfillment_status = 'pending_export'

    def sale_order_refunded(self):
        self.order_status = 'cancelled'
        self.fulfillment_status = 'pending_export'
        self.payment_status = 'refunded'
        for line in self.order_line:
            line.sol_status = 'cancelled'
            line.sol_payment_status = 'refunded'
            line.sol_fulfillment_status = 'pending_export'

    def update_sale_order_status(self):
        if not self.woo_status:
            return False
        if self.woo_status == 'refunded':
            self.sale_order_refunded()
        elif self.woo_status == 'processing':
            self.sale_order_processing()
        elif self.woo_status == 'on-hold':
            self.sale_order_on_hold()
        elif self.woo_status == 'pending':
            self.sale_order_on_pending()
        elif self.woo_status in ['cancelled', 'failed']:
            self.sale_order_on_other()
        return True

    def write(self, vals):
        res = super(SaleOrder, self).write(vals)
        if vals.get("woo_status"):
            for rec in self:
                rec.update_order_status_by_woo(vals.get("woo_status"))
        return res

    def update_order_status_by_woo(self, woo_status):
        if self.order_status not in ['pending', 'cancelled']:
    ...
    def woo_convert_dates_by_timezone(self, instance, from_date, to_date):
        ...
        if not from_date:
            if instance.last_order_import_date:
                from_date = instance.last_order_import_date - timedelta(days=30)
            else:
                from_date = fields.Datetime.now() - timedelta(days=30)
    ...
    def import_woo_orders_with_cron(self, woo_instance):
        woo_object = self.env['woo.instance.ept'].search([('id', '=', woo_instance)])
        self.with_delay(description=f'Update sale order cron {woo_object.name}').import_woo_orders(woo_instance=woo_instance, from_date="",
                                                                                                   to_date="", order_type='cron')
    ...
    def import_woo_orders(self, woo_instance, from_date="", to_date="", order_type=""):
        ...
        if order_type == 'cron':
            params["status"] = ",".join(map(str, woo_instance.import_order_status_ids.mapped("status") +
    ...
    def create_woo_log_lines(self, message, common_log_book_id=False, queue_line=False, order_data=False):
        """
        ...
        if order_data:
            log_line_vals.update({
                'order_ref': order_data.get('number'),
                'date_created': order_data.get('date_created').replace('T', ' ')
            })
    ...
    def update_woo_order_vals(self, order_data, woo_order_number, woo_instance, workflow_config, shipping_partner):
        ...
        vals = {
            ...
            "transaction_authorization": order_data.get("transaction_id"),
            ...
            "total_discounts": order_data.get("discount_total"),
            "shipping_fee": order_data.get("shipping_total"),
        }
        return vals
    ...
    def prepare_woo_order_vals(self, order_data, woo_instance, partner, billing_partner, shipping_partner,
        ...
        Task_id: 165893
        """
        date_created_gmt = order_data.get("date_created_gmt")
        if not date_created_gmt:
            date_created_gmt = datetime.strftime(datetime.now(), '%Y-%m-%dT%H:%M:%S')
        date_paid_gmt = order_data.get("date_paid_gmt")
        order_date = datetime.strptime(date_created_gmt.replace("T", " "), '%Y-%m-%d %H:%M:%S') - timedelta(hours=15)
        ...
        total_line_items_price = float(order_data.get("total", 0.0)) + float(order_data.get("discount_total", 0.0)) - float(
            order_data.get("shipping_total", 0.0))
        woo_order_vals.update({"total_line_items_price": total_line_items_price})
        woo_order_vals.update({"date_created_gmt": date_created_gmt.replace("T", " ")})
        if date_paid_gmt:
            date_paid = datetime.strptime(date_paid_gmt.replace("T", " "), '%Y-%m-%d %H:%M:%S') - timedelta(hours=15)
            date_paid = date_paid.strftime('%Y-%m-%d %H:%M:%S')
            woo_order_vals.update({"paid_at": date_paid,
                                   "date_paid_gmt": date_paid_gmt.replace("T", " ")})
    ...
    def create_woo_order_line(self, line_id, product, quantity, price, taxes, tax_included, woo_instance,
                              is_shipping=False, is_extra_line=False):
        """
    ...
    def woo_create_discount_line(self, order_line, tax_included, woo_instance, taxes, order_line_id):
        ...
        if line_discount > 0:
            ...
            discount_line = self.create_woo_order_line(False, woo_instance.discount_product_id, 1, line_discount * -1,
                                                       taxes, tax_included, woo_instance, is_extra_line=True)
    ...
    def convert_order_date_gmt_8(self, date_created):
        # convert create_date to gmt-8
        if date_created:
            date_order_gmt = datetime.strptime(date_created.replace("T", " "), '%Y-%m-%d %H:%M:%S') - timedelta(hours=15)
    ...
    def create_woo_sale_order_lines(self, queue_line, order_data, tax_included, common_log_book_id, woo_taxes,
        ...
        for order_line in order_data.get("line_items"):
            ...
            woo_product = self.find_or_create_woo_product(queue_line, order_line, common_log_book_id,
                                                          is_process_from_queue)
            if not woo_product:
                message = "Product [%s][%s] not found for Order %s - Create at %s" % (
                    order_line.get("sku"), order_line.get("name"), order_data.get('number'), order_data.get('date_created').replace('T', ' '))
                self.create_woo_log_lines(message, common_log_book_id, queue_line, order_data)
                return False
            product = woo_product.product_id
            product_url = woo_product.product_url
            ...
            if order_line_id and sku:
                order_line_id.write({'product_sku': sku, 'product_url': product_url})
    ...
    def verify_order_for_payment_method(self, order_data):
        ...
        if not total or not total_discount:
            return False
    ...
    def create_woo_orders(self, queue_lines, common_log_book_id):
        ...
        for queue_line in queue_lines:
            ...
            if order_data.get("date_created_gmt"):
            ...
            if existing_order:
                # woo_status = order_data.get("status")
                date_paid_gmt = order_data.get("date_paid_gmt")
                for order_id in existing_order:
                    order = self.env['sale.order'].browse(order_id)
                    order_update_vals = {}
                    if not order.woo_trans_id and order_data.get("transaction_id", False):
                        order_update_vals.update({
                            "woo_trans_id": order_data.get("transaction_id"),
                            "transaction_authorization": order_data.get("transaction_id")})

                    if date_paid_gmt and not order.paid_at:
                        date_paid = datetime.strptime(date_paid_gmt.replace("T", " "), '%Y-%m-%d %H:%M:%S') - timedelta(
                            hours=15)
                        date_paid = date_paid.strftime('%Y-%m-%d %H:%M:%S')
                        order_update_vals.update({
                            "paid_at": date_paid,
            ...
            # vietnt
            sale_order.processing_new_order_in_odoo()
        ...
        for shipping_line in order_data.get("shipping_lines"):
            ...
            if delivery_method:
                ...
                self.create_woo_order_line(shipping_line.get("id"), shipping_product, 1, total_shipping, taxes,
                                           tax_included, self.woo_instance_id, True, is_extra_line=True)
        ...
        for fee_line in order_data.get("fee_lines"):
            ...
            if total_fee:
                ...
                self.create_woo_order_line(fee_line.get("id"), self.woo_instance_id.fee_product_id, 1, total_fee, taxes,
                                           tax_included, self.woo_instance_id, is_extra_line=True)
    ...
    def update_woo_order_status_with_cron(self, woo_instance):
        self.with_delay(description='Sync sale order status cron').update_woo_order_status(woo_instance=woo_instance)
    ...
    def update_woo_one_order_status(self):
        if not self.woo_order_id or self.woo_status == 'completed' or not self.woo_instance_id:
            return False
        order_line = self.order_line.filtered(lambda line: not line.is_extra_line)
        if not all([line.woo_fulfilled for line in order_line]):
            return False
        instance = self.woo_instance_id
        wc_api = instance.woo_connect()
        try:
            response = wc_api.put('orders/%s' % self.woo_order_id, {'status': 'completed'})
            ...
            if message:
                ...
                if not order.woo_trans_id and order_data.get("transaction_id", False):
                    order_vals.update({"woo_trans_id": order_data.get("transaction_id"),
                                       "transaction_authorization": order_data.get("transaction_id")})
                date_paid_gmt = order_data.get("date_paid_gmt")
                if not order.paid_at and date_paid_gmt:
                    date_paid = datetime.strptime(date_paid_gmt.replace("T", " "), '%Y-%m-%d %H:%M:%S') - timedelta(
                        hours=15)
                    date_paid = date_paid.strftime('%Y-%m-%d %H:%M:%S')
                    order_vals.update({
    ...
    def complete_woo_order(self):
        ...
        """
        if not self.state == "sale" and self.state not in ['cancel', 'done']:
            self.action_confirm()
    ...
    def processing_in_woo(self):
        instance = self.woo_instance_id
        wcapi = instance.woo_connect()
        info = {'status': 'processing', 'id': self.woo_order_id}
        try:
        ...
        _logger.info("Processing sale order: %s in WooCommere store with Woo commerce order id: %s", self.name,
                     self.woo_order_id)
        ...
        if response.status_code in [200, 201]:
            ...
            raise UserError(_("Error in Processing Order %s", response.content))

    def update_status_refunded_in_woo(self):
        instance = self.woo_instance_id
        wcapi = instance.woo_connect()
        info = {'status': 'refunded', 'id': self.woo_order_id}
        try:
            response = wcapi.post('orders/batch', {'update': [info]})
        except Exception as error:
            raise UserError(_("Something went wrong while Updating Orders.\n\nPlease Check your Connection and "
        ...
        if response.status_code in [200, 201]:
            self.write({'woo_status': 'refunded'})
        else:
            raise UserError(_("Error in Processing Order %s", response.content))

    def process_refunded_status(self):
        res = super(SaleOrder, self).process_refunded_status()
        if res.get("payment_status") == "refunded":
            self.update_status_refunded_in_woo()
        return res

    def action_processing_sale_order(self):
        res = super(SaleOrder, self).action_processing_sale_order()
        self.processing_in_woo()
        return res

    def cancel_from_cancel_request(self):
        res = super(SaleOrder, self).cancel_from_cancel_request()
        self.cancel_in_woo_store()
        return res

    def cancel_in_woo_store(self):
        sale_order_obj = self.env['sale.order']
        ...
        if response.status_code in [200, 201]:
            order.write({'cancelled_in_woo': True, 'woo_status': 'cancelled'})
        else:
...
class SaleOrderLine(models.Model):
    ...
    woo_instance_id = fields.Many2one("woo.instance.ept", "Woo Instance", related='order_id.woo_instance_id', store=True)
    woo_line_id = fields.Char()
    woo_fulfilled = fields.Boolean(string='Woo Fulfilled')
    woo_fulfill_id = fields.Char(string='Woo Fullfill ID')
    endpoint_paypal_tracking = fields.Char()
    woo_status = fields.Selection(related='order_id.woo_status', store=True)
    primary_domain = fields.Char(related="order_id.primary_domain", store=True)

    def sale_order_line_processing(self):
        self.sol_status = 'processing'
        self.sol_payment_status = 'paid'
        self.sol_fulfillment_status = 'unexported'

    def sale_order_line_on_hold(self):
        self.sol_status = 'on_hold'
        self.sol_payment_status = False
        self.sol_fulfillment_status = 'pending_export'

    def sale_order_line_on_pending(self):
        self.sol_status = 'pending_payment'
        self.sol_payment_status = 'pending'
        self.sol_fulfillment_status = 'pending_export'

    def sale_order_line_on_other(self):
        self.sol_status = 'failed'
        self.sol_payment_status = False
        self.sol_fulfillment_status = 'pending_export'

    def sale_order_line_refund(self):
        self.sol_status = 'cancelled'
        self.sol_payment_status = 'refunded'
        self.sol_fulfillment_status = 'pending_export'

    def update_sale_order_line_status(self):
        if not self.order_id.woo_status:
            return False
        if self.order_id.woo_status == 'refunded':
            self.sale_order_line_refund()
        elif self.order_id.woo_status == 'processing':
            self.sale_order_line_processing()
        elif self.order_id.woo_status == 'on-hold':
            self.sale_order_line_on_hold()
        elif self.order_id.woo_status == 'pending':
            self.sale_order_line_on_pending()
        elif self.order_id.woo_status in ['cancelled', 'failed']:
            self.sale_order_line_on_other()
        return True
    ...
    def create(self, vals_list):
        ...
        if res.order_id:
            res.update_sale_order_line_status()
            if res.is_extra_line or res.order_id.order_status not in ['processing', 'pending']:
                return res
            if res.order_id.payment_gateway_id.code != 'paypal_express':
                res.check_order_duplicate()
            res.order_id.compute_delivery_address_error()
        return res

    def sync_tracking_number_to_woo(self):
        if not self.woo_instance_id:
            return False
        wc_api = self.woo_instance_id.with_context(woo_version='wc-shipment-tracking/v3').woo_connect()
        woo_order_id = self.order_id.woo_order_id
        url = f"orders/{woo_order_id}/shipment-trackings"
        data = {
            "custom_tracking_provider": self.carrier_id.name,
            "custom_tracking_link": self.carrier_url,
            "tracking_number": self.tracking_number,
            'date_shipped': datetime.today().strftime('%Y-%m-%d')
        }
        try:
            res = wc_api.post(url, data)
            content = res.json()
            if res.status_code not in [200, 201]:
                return False
            woo_fulfill_id = content.get("tracking_id")
            self.write({"woo_fulfill_id": woo_fulfill_id, "woo_fulfilled": True})
            self.order_id.update_woo_one_order_status()

        except Exception as error:
            raise UserError(_("Something went wrong sync tracking number to Woo.\n\nPlease Check your Connection "
                              "and Instance Configuration.\n\n" + str(error)))

    def delete_tracking_number_woo(self):
        woo_order_id = self.order_id.woo_order_id
        woo_fulfill_id = self.woo_fulfill_id
        if not self.woo_instance_id or not woo_order_id or not woo_fulfill_id:
            return False
        wc_api = self.woo_instance_id.with_context(woo_version='wc-shipment-tracking/v3').woo_connect()
        url = f"orders/{woo_order_id}/shipment-trackings/{woo_fulfill_id}"
        try:
            res = wc_api.delete(url)
            if res.status_code not in [200, 201]:
                return False
        except Exception as error:
            raise UserError(_("Something went wrong sync tracking number to Woo.\n\nPlease Check your Connection "
    ...
    def update_tracking_number_woo(self):
        self.delete_tracking_number_woo()
        self.sync_tracking_number_to_woo()

    def process_new_tracking_number(self):
        if not self.tracking_number:
            return False
        # self.trackingmore_create_tracking()
        self.update_tracking_order_line_17track()
        if self.shopbase_order_id and (self.trackingmore_registed or self.register_17track):
            self.sync_tracking_number_to_shopbase()
        if self.woo_line_id and (self.trackingmore_registed or self.register_17track):
            self.sync_tracking_number_to_woo()

    def process_update_tracking_number(self):
        if not self.tracking_number or not self.shopbase_fulfill_id:
            return False
        # self.trackingmore_create_tracking()
        self.update_tracking_order_line_17track()
        if self.shopbase_order_id and (self.trackingmore_registed or self.register_17track):
            self.update_tracking_number_shopbase()
        if self.woo_line_id and (self.trackingmore_registed or self.register_17track):
            self.update_tracking_number_woo()
    ...
    def write(self, vals):
        ...
        if not vals.get("tracking_number"):
            return res
        for rec in self:
            rec.trackingmore_detect_carriers()
            rec.sale_tracking_url = f"https://t.17track.net/en#nums={rec.tracking_number}"
            if not rec.woo_line_id and not rec.shopbase_order_id:
                continue
            if (rec.woo_fulfill_id or rec.shopbase_order_id) and (rec.trackingmore_registed or rec.register_17track):
                rec.with_delay(
                    description='Process update tracking number!').process_update_tracking_number()
            else:
                rec.with_delay(
                    description='Process register tracking number!').process_new_tracking_number()
            rec.with_delay(description='Process register tracking in Paypal!').process_register_tracking_paypal()
            rec.with_delay(
                description=f'Send email tracking number process {rec.tracking_number} - {rec.order_id.name}!').send_email_tracking_number_process()
        return res

    def send_email_tracking_number_process(self):
        if self.is_line_send_mail_tracking:
            # self.order_id.action_send_mail_order('mail_template_change_tracking_number')
            return True
        self.is_line_send_mail_tracking = True
        if self.order_id.time_send_mail_tracking:
            return False
        self.order_id.time_send_mail_tracking = fields.Datetime.now()
        if len(self.order_id.order_line.filtered(lambda r: r.is_extra_line is False).ids) == 1:
            self.send_email_tracking_number()
            return True
        self.with_delay(description=f'Process send email tracking order', eta=60).send_email_tracking_number()

    def send_email_tracking_number(self):
        self.order_id.time_send_mail_tracking = False
        self.order_id.action_send_mail_order('mail_template_add_tracking_number')
        self.order_id.send_message_sms(self.order_id, self.order_id.partner_shipping_id, 'new_tracking')
        return True

    def process_register_tracking_paypal(self):
        if not self.order_id.payment_gateway_id.account_paypal_payment:
            return False
        if self.order_id.account_paypal_payment != self.order_id.payment_gateway_id.account_paypal_payment:
            self.order_id.account_paypal_payment = self.order_id.payment_gateway_id.account_paypal_payment
        if not self.endpoint_paypal_tracking:
            register_status = self.order_id.paypal_register_create_tracking(self)
        else:
            register_status = self.order_id.paypal_register_update_tracking(self)
        if register_status:
            self.endpoint_paypal_tracking = f'/v1/shipping/trackers/{self.order_id.transaction_authorization}-{self.tracking_number}'
