...
class WooProductTemplateEpt(models.Model):
    ...
    def create(self, vals):
        res = super(WooProductTemplateEpt, self).create(vals)
        export_product_model = res.env['woo.process.import.export']
        export_product = export_product_model.with_context(process='export_products', active_ids=[res.id]).create({
            'woo_basic_detail': True,
            'woo_is_set_image': True,
            'woo_is_set_price': True,
            'woo_publish': 'publish'
        })
        export_product.woo_export_products()
        return res
    ...
    @api.model
    def prepare_woo_template_vals(self, template_info, odoo_template_id, import_for_order, woo_instance,
                                  common_log_book_id):
        """
        ...
        """
        template_data = template_info.copy()
        if import_for_order:
    ...
    def prepare_woo_variant_vals(self, woo_instance, variant, template_title=""):
        ...
        variant_info = {
            ...
            "exported_in_woo": True,
            "product_url": variant.get("permalink", base_url),
    ...
    def get_gallery_images(self, instance, woo_template, template):
        ...
        position = 0
        gallery_img_keys = {}
        gallery_images = woo_template.woo_image_ids
    ...
    def prepare_product_variant_dict(self, instance, template, data, basic_detail, update_price,
        ...
        for variant in template.woo_product_ids:
            regular_price, sale_price = variant.regular_price, variant.sale_price
            ...
            if update_price:
                ...
                info.update({'regular_price': str(regular_price), 'sale_price': str(sale_price)})
            ...
            if template.woo_tmpl_id != variant.variant_id:
                ...
                if update_price:
                    data.update({'regular_price': str(regular_price), 'sale_price': str(sale_price)})
                flag = True
...
class ProductProductEpt(models.Model):
    ...
    created_at = fields.Datetime()
    updated_at = fields.Datetime()
    woo_is_manage_stock = fields.Boolean("Is Manage Stock?", default=False,
    ...
    regular_price = fields.Float(string='Price')
    sale_price = fields.Float(string='Compare Price')
