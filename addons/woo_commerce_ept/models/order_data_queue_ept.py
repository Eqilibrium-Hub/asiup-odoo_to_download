...
class WooOrderDataQueueEpt(models.Model):
    ...
    def import_order_queue(self):
        self.env['woo.manual.queue.process.ept'].with_context(queue_type='order', active_ids=[self.id]).process_queue_manually()
    ...
    def create(self, vals):
        ...
        res = super(WooOrderDataQueueEpt, self).create(vals)
        res.with_delay(description='Sync sale order from Woo').import_order_queue()
        return res
