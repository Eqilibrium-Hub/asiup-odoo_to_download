...
class WooInstanceConfig(models.TransientModel):
    ...
    def woo_test_connection(self):
        ...
        self.create_cron_job_for_auto_update_order(instance)
        return {
            'type': 'ir.actions.client',
            'tag': 'reload',
        }

    def create_cron_job_for_auto_update_order(self, instance):
        woo_cron_obj = self.env["woo.cron.configuration.ept"]
        cron_exist = woo_cron_obj.search([('woo_instance_id', '=', instance.id)])
        if cron_exist:
            return
        woo_import_interval_number = 30
        woo_import_interval_type = 'minutes'
        woo_import_next_execution = fields.Datetime.now() + timedelta(minutes=1)
        woo_import_user_id = 2
        woo_cron = woo_cron_obj.create({
            'woo_instance_id': instance.id,
            'woo_auto_import_order': True,
            'woo_auto_update_order_status': True,
            'woo_import_order_interval_number': woo_import_interval_number,
            'woo_import_order_interval_type': woo_import_interval_type,
            'woo_import_order_next_execution': woo_import_next_execution,
            'woo_import_order_user_id': woo_import_user_id,
            'woo_update_order_status_interval_number': woo_import_interval_number,
            'woo_update_order_status_interval_type': woo_import_interval_type,
            'woo_update_order_status_next_execution': woo_import_next_execution,
            'woo_update_order_status_user_id': woo_import_user_id,
        })
        woo_cron.save()
...
class ResConfigSettings(models.TransientModel):
    ...
    woo_attribute_type = fields.Selection([("select", "Select"), ("text", "Text")],
                                          string="Attribute Type For Export Operation", default="text",
