...
class Webhook(http.Controller):
    ...
    def update_order_webhook(self):
        ...
        if instance.active:
            ...
            if order:
                sale_order_obj.sudo().with_delay(
                    description='Update sale order woo with webhook').process_order_via_webhook(res, instance, True)
                if delete_webhook:
                    _logger.info("Cancelled order %s of %s via Webhook as deleted in Woo Successfully", order.name,
                                 instance.name)
            elif res.get("status") in ['pending', 'completed', 'processing', 'on-hold', 'cancelled', 'refunded', 'failed', 'trash']:
            # else:
                sale_order_obj.sudo().with_delay(
                    description='Update sale order woo with webhook').process_order_via_webhook(res, instance)
        return
