...
class QueueJob(models.Model):
    ...
    def job_requeue_failed_job_in_cron(self):
        time_requeue = fields.datetime.today()
        self.with_delay(description='Requeue Failed Job before').cron_job_requeue_fail_job(time_requeue)

    def cron_job_requeue_fail_job(self, time_requeue):
        time_requeue = time_requeue - timedelta(minutes=30)
        failed_job_ids = self.search([('state', '=', 'failed'), ('date_started', '>', time_requeue)])
