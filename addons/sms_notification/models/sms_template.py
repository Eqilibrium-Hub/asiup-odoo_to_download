...
class SmsTemplate(models.Model):
    ...
    # condition = fields.Selection([('new_tracking', 'Add Tracking Number'),
    #                               ('change_tracking', 'Change Tracking Number'),
    ...
    def onchange_condition(self):
        for obj in self:
            # if obj.condition == 'order_success':
    ...
    def send_sms_using_template(self, mob_no, sms_tmpl, sms_gateway=None, obj=None):
        ...
        if mob_no and sms_tmpl:
            ...
            sms_sms_obj = self.env["wk.sms.sms"].create({
                ...
                'partner_id': obj.partner_shipping_id.id if obj else False,
                'to': mob_no,
                'group_type': 'individual',
                'auto_delete': sms_tmpl.auto_delete,
                'msg': sms_tmpl.with_context(ctx).get_body_data(obj, obj.partner_shipping_id) if obj else sms_tmpl.sms_body_html,
                'template_id': False
            })
